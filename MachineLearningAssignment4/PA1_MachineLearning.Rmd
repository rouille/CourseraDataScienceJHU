...
```
The training data set contains `r dim(training)[1]` observations of `r dim(training)[2]` variables. The testing data set encloses `r dim(testing)[1]` observations of the same variables.
...
After the above cleaning procedure, the training data set is made of `r dim(training)[1]` observations of `r dim(training)[2]` variables. We want to partition this data set in a training (60% of the original data) and validation (remaining 40%) data set.
...
```{r Vaiables of Importance, warning = FALSE, message = FALSE, fig.width = 5, fig.height = 8}
